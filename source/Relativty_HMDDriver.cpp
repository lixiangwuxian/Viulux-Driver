...
void Relativty::HMDDriver::retrieve_device_quaternion_packet_threaded() {//这个函数用来接收设备的缓冲区并转换为所需的四元数
	...
	if (!this->start_tracking_server) {
		...
		NOLOData hmdpos;
		hmdpos=GetNoloData();
		this->vector_xyz[1] = hmdpos.hmdData.HMDPosition.y;//没追踪，那就当3自由度头盔用，高度垫高点
	...
	while (this->retrieve_quaternion_isOn) {
		//result = hid_read(this->handle, packet_buffer, 64); //读USB缓冲区的指令。Result should be greater than 0.
		if (result > 0) {
			if (m_bIMUpktIsDMP) {
