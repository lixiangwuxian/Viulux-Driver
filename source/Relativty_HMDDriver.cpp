...
void Relativty::HMDDriver::retrieve_device_quaternion_packet_threaded() {//这个函数用来接收四元数
	...
	while (this->retrieve_quaternion_isOn) {
		...
		if (m_BRecentering)
		{
			...
			{
				real_head_yaw = PI / 2 + atan((HandPosStd.z - HeadPosStd.z) / (HandPosStd.x - HeadPosStd.x));//取头手连线与头部朝向连线之间的夹角
			}
			Head_Euler = HeadRot.GetEulerAngle();//转换四元数至欧拉角
